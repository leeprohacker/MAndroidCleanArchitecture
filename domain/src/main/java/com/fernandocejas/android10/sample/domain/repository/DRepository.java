...
public interface DRepository {
  ...
  Observable<IHome> getHome();

  Observable<List<DDataItem>>  getGiphyTrend(final  int offset , final  int limit);

  void onLiked(String idPost , boolean isLike);

   Observable<DComment> postComment(String content , String idPost);

   Observable<List<DComment>> getComments(String idPost);
